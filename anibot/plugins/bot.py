    ...
    if user in OWNER or (
        ...
        if len(cmd)==1:
            x = await message.reply_text(
                'Nᴏ ᴄᴏᴍᴍᴀɴᴅ sᴘᴇᴄɪғɪᴇᴅ ᴛᴏ ʙᴇ ᴅɪsᴀʙʟᴇᴅ!!!'
            )
        ...
        if set(cmd[1].split()).issubset(CMD):
            ...
            if find_gc is None:
                if enable:
                    x = await message.reply_text('Cᴏᴍᴍᴀɴᴅ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ!!!')
                    await asyncio.sleep(5)
                    await x.delete()
                    return
                await DC.insert_one({'_id': gid, 'cmd_list': cmd[1]})
                x = await message.reply_text("Cᴏᴍᴍᴀɴᴅ ᴅɪsᴀʙʟᴇᴅ!!!")
                await asyncio.sleep(5)
                await x.delete()
                return
            else:
                ...
                if set(cmd[1].split()).issubset(ocls.split()):
                    if enable:
                        if len(ocls.split())==1:
                            ...
                            x = await message.reply_text("Cᴏᴍᴍᴀɴᴅ ᴇɴᴀʙʟᴇᴅ!!!")
                            await asyncio.sleep(5)
                            await x.delete()
                            return
                        ncls = ocls.split()
            ...
            await message.reply_text("Hᴇᴇ, ɪs ᴛʜᴀᴛ ᴀ ᴄᴏᴍᴍᴀɴᴅ?!")
...
async def list_disabled(client: Client, message: Message, mdata: dict):
    ...
    if find_gc is None:
        await message.reply_text("Nᴏ ᴄᴏᴍᴍᴀɴᴅs ʜᴀs ᴅɪsᴀʙʟᴇᴅ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ!!!")
    else:
        lscmd = find_gc['cmd_list'].replace(" ", "\n")
        await message.reply_text(
f"""Lɪsᴛ ᴏғ ᴄᴏᴍᴍᴀɴᴅs ᴅɪsᴀʙʟᴇᴅ ɪɴ **{mdata['chat']['title']}**
...
async def db_cleanup(client: Client, message: Message, mdata: dict):
    ...
    entries = ""
    st = datetime.now()
    x = await message.reply_text("Sᴛᴀʀᴛɪɴɢ ᴅᴀᴛᴀʙᴀsᴇ ᴄʟᴇᴀɴᴜᴘ ɪɴ 5 sᴇᴄᴏɴᴅs")
    et = datetime.now()
    pt = (et-st).microseconds / 1000
    await asyncio.sleep(5)
    await x.edit_text("Cʜᴇᴄᴋɪɴɢ 1sᴛ ᴄᴏʟʟᴇᴄᴛɪᴏɴ!!!")
    ...
    await asyncio.sleep(5)
    await x.edit_text("Cʜᴇᴄᴋɪɴɢ 2ɴᴅ ᴄᴏʟʟᴇᴄᴛɪᴏɴ!!!")
...
ID: `{user}`""",
        ...
        await client.send_message(
            ...
            text=(
                f"**Kᴏɴ'ɴɪᴄʜɪᴡᴀ!!!**\n"
                +f"**I'ᴍ ****{bot.first_name}**** ʙᴏᴛ ᴀɴᴅ I ᴄᴀɴ ʜᴇʟᴘ ʏᴏᴜ ɢᴇᴛ ɪɴғᴏ ᴏɴ **"
                +f"**Aɴɪᴍᴇs, Mᴀɴɢᴀs, Cʜᴀʀᴀᴄᴛᴇʀs, Aɪʀɪɴɢs, Sᴄʜᴇᴅᴜʟᴇs, Wᴀᴛᴄʜ **"
                +f"**Oʀᴅᴇʀs ᴏғ Aɴɪᴍᴇs, ᴇᴛᴄ.**"
                +f"\n\n**Fᴏʀ ᴍᴏʀᴇ ɪɴғᴏ sᴇɴᴅ /help ɪɴ ʜᴇʀᴇ.**"
                +f"**Iғ ʏᴏᴜ ᴡɪsʜ ᴛᴏ ᴜsᴇ ᴍᴇ ɪɴ ᴀ ɢʀᴏᴜᴘ sᴛᴀʀᴛ ᴍᴇ ʙʏ **"
                +f"**/start{BOT_NAME} ᴄᴏᴍᴍᴀɴᴅ ᴀғᴛᴇʀ ᴀᴅᴅɪɴɢ ᴍᴇ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ.**")
        )
    else:
        ...
        await client.send_message(gid, text="**ᴋᴏᴍɪ ɪs ᴏɴʟɪɴᴇ ᴀɴᴅ ʜᴀs ʙᴇᴇɴ sᴜᴄᴄᴇssғᴜʟʟʏ sᴛᴀʀᴛᴇᴅ!!! **")
...
async def help_(client: Client, message: Message, mdata: dict):
    ...
    try:
        ...
        await client.send_message(
            gid,
            text="Cʟɪᴄᴋ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ ғᴏʀ ʙᴏᴛ ʜᴇʟᴘ",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Hᴇʟᴘ", url=f"https://t.me/{bot_us}/?start=help")]])
        )
        return
    buttons = help_btns(id_)
    text='''**Bᴀsɪᴄ Cᴏᴍᴍᴀɴᴅs:**
Usᴇ /ping ᴏʀ !ping ᴄᴍᴅ ᴛᴏ ᴄʜᴇᴄᴋ ɪғ ʙᴏᴛ ɪs ᴏɴʟɪɴᴇ
Usᴇ /start ᴏʀ !start ᴄᴍᴅ ᴛᴏ sᴛᴀʀᴛ ʙᴏᴛ ɪɴ ɢʀᴏᴜᴘ ᴏʀ ᴘᴍ
Usᴇ /help ᴏʀ !help ᴄᴍᴅ ᴛᴏ ɢᴇᴛ ɪɴᴛᴇʀᴀᴄᴛɪᴠᴇ ʜᴇʟᴘ ᴏɴ ᴀᴠᴀɪʟᴀʙʟᴇ ʙᴏᴛ ᴄᴍᴅs
Usᴇ /feedback ᴄᴍᴅ ᴛᴏ ᴄᴏɴᴛᴀᴄᴛ ʙᴏᴛ ᴏᴡɴᴇʀ'''
    if id_ in OWNER:
        await client.send_message(gid, text=text, reply_markup=buttons)
        await client.send_message(
            gid,
            text="""Oᴡɴᴇʀs / Sᴜᴅᴏs ᴄᴀɴ ᴀʟsᴏ ᴜsᴇ

- __/term__ `ᴛᴏ ʀᴜɴ ᴀ ᴄᴍᴅ ɪɴ ᴛᴇʀᴍɪɴᴀʟ`
- __/eval__ `ᴛᴏ ʀᴜɴ ᴀ ᴘʏᴛʜᴏɴ ᴄᴏᴅᴇ ʟɪᴋᴇ `__/eval print('UᴡU')__` `
- __/stats__ `ᴛᴏ ɢᴇᴛ sᴛᴀᴛs ᴏɴ ʙᴏᴛ ʟɪᴋᴇ ɴᴏ. ᴏғ ᴜsᴇʀs, ɢʀᴘs ᴀɴᴅ ᴀᴜᴛʜᴏʀɪsᴇᴅ ᴜsᴇʀs`
- __/dbcleanup__ `ᴛᴏ ʀᴇᴍᴏᴠᴇ ᴏʙsᴏʟᴇᴛᴇ/ᴜsᴇʟᴇss ᴇɴᴛʀɪᴇs ɪɴ ᴅᴀᴛᴀʙᴀsᴇ`

Aᴘᴀʀᴛ ғʀᴏᴍ ᴀʙᴏᴠᴇ sʜᴏᴡɴ ᴄᴍᴅs"""
        )
    else:
        if gid==id_:
            await client.send_message(gid, text=text, reply_markup=buttons)
...
async def connect_(client: Client, message: Message, mdata: dict):
    ...
    try:
        ...
        await client.send_message(
            gid,
            text="Gᴏ ᴛᴏ ʙᴏᴛ ᴘᴍ ᴛᴏ ᴄᴏɴɴᴇᴄᴛ ᴄʜᴀɴɴᴇʟ",
            reply_markup=InlineKeyboardMarkup(
                [[
                    InlineKeyboardButton(
                        "Bᴏᴛ PM", url=f"https://t.me/{bot_us}"
                    )
                ]]
            )
        )
    ...
    if gid==id_:
        ...
        try:
            ...
            return await client.send_message(
                gid,
                text=(
                    "Pʟᴇᴀsᴇ ᴘʀᴏᴠɪᴅᴇ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ ɪᴅ ʏᴏᴜ ᴡɪsʜ ᴛᴏ ᴄᴏɴɴᴇᴄᴛ!!!"
                    +"\nExᴀᴍᴘʟᴇ: /connect -100xxxxxxxxx"
                )
            )
        if not "-100" in channel:
            return await client.send_message(
                gid, text="Pʟᴇᴀsᴇ ᴇɴᴛᴇʀ ᴛʜᴇ ғᴜʟʟ ᴄʜᴀɴɴᴇʟ ID!!!"
            )
        if  data[0]=='connect':
            if (await CC.find_one({"_id": str(channel)})):
                await client.send_message(
                    gid,
                    text=(
                        "Cʜᴀɴɴᴇʟ ᴀʟʀᴇᴀᴅʏ ᴄᴏɴɴᴇᴄᴛᴇᴅ"
                        +"\nIғ sᴏᴍᴇᴏɴᴇ ᴇʟsᴇ ʜᴀs ᴀᴄᴄᴇss ᴛᴏ ɪᴛ ᴡʜᴏ ᴅᴏᴇsɴ'ᴛ ᴏᴡɴ "
                        +"ᴛʜᴇ ᴄʜᴀɴɴᴇʟ, ᴄᴏɴᴛᴀᴄᴛ @GenAnimeOwner"
                    )
                )
                return
            await CC.insert_one({"_id": str(channel), "usr": id_})
            await client.send_message(
                gid, text="Sᴜᴄᴄᴇssғᴜʟʟʏ ᴄᴏɴɴᴇᴄᴛᴇᴅ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ"
            )
        else:
            ...
            if k and k['usr']==id_:
                ...
                await client.send_message(
                    gid, text="Sᴜᴄᴄᴇssғᴜʟʟʏ ᴅɪsᴄᴏɴɴᴇᴄᴛᴇᴅ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ"
                )
            else:
                await client.send_message(gid, text="Cʜᴀɴɴᴇʟ ɴᴏᴛ ᴄᴏɴɴᴇᴄᴛᴇᴅ")
    else:
...
async def help_dicc_parser(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    btn = InlineKeyboardMarkup(
        [[InlineKeyboardButton("Bᴀᴄᴋ", callback_data=f"hlplist_{user}")]]
    )
...
async def feed_(client: Client, message: Message, mdata: dict):
    ...
    await client.send_message(
        mdata['chat']['id'],
        f"Fᴏʀ ɪssᴜᴇs ᴏʀ ǫᴜᴇʀɪᴇs ᴘʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ "
        +f"@{owner} ᴏʀ ᴊᴏɪɴ @LordShadowSama"
    )
