...
admin_error_msg = "Pʟᴇᴀsᴇ ɢɪᴠᴇ ʙᴏᴛ Pɪɴ Mᴇssᴀɢᴇ ᴀɴᴅ Dᴇʟᴇᴛᴇ Mᴇssᴀɢᴇ ᴘᴇʀᴍɪssɪᴏɴs ᴛᴏ ᴘɪɴ ɴᴇᴡ ʜᴇᴀᴅʟɪɴᴇs!!!\nOʀ ʏᴏᴜ ᴄᴀɴ ᴅɪsᴀʙʟᴇ Pɪɴ ᴀɴᴅ Uɴᴘɪɴ ᴏᴘᴛɪᴏɴs ɪɴ /settings ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ sᴛᴏᴘ sᴇᴇɪɴɢ ᴛʜɪs ᴍᴇssᴀɢᴇ"
    ...
    try:
        ...
        for i in lc:
            if len(i[0])==2:
                ...
                text = f'{i[0][0]} ᴊᴜsᴛ ᴀɪʀᴇᴅ'
                msgslc.append([text, i[1]])
        for i in list(clc.keys()):
            if len(clc[i])>1:
                aep = [clc[i][len(clc[i])-1][0], clc[i][0][0]]
                text = f'\nEᴘɪsᴏᴅᴇ {min(aep)} - {max(aep)} ᴏғ {i} ᴊᴜsᴛ ᴀɪʀᴇᴅ'
            else:
                text = f'\nEᴘɪsᴏᴅᴇ {clc[i][0][0]} ᴏғ {i} ᴊᴜsᴛ ᴀɪʀᴇᴅ'
        ...
        for i in msgslc:
            async for id_ in AR_GRPS.find():
                btn = InlineKeyboardMarkup([[
                    InlineKeyboardButton("Mᴏʀᴇ Iɴғᴏ", url=i[1])
                ]])
                try:
    ...
    if await SP_GRPS.find_one() is not None:
        for i in msgssp:
            async for id_ in SP_GRPS.find():
                btn = InlineKeyboardMarkup([[
                    InlineKeyboardButton("Dᴏᴡɴʟᴏᴀᴅ", url=i[1])
                ]])
                try:
                    await anibot.send_message(
                        id_['_id'], i[0], reply_markup=btn
    ...
    if await HD_GRPS.find_one() is not None:
        for i in msgslch:
            async for id_ in HD_GRPS.find():
                var_dict = {}
                btn = InlineKeyboardMarkup([[
                    InlineKeyboardButton("Mᴏʀᴇ Iɴғᴏ", url=i[2]),
                    InlineKeyboardButton("Sᴏᴜʀᴄᴇ", url=i[3]),
                ]])
                try:
                    try:
