...
"""Pʟᴇᴀsᴇ ɢɪᴠᴇ ᴀ ǫᴜᴇʀʏ ᴛᴏ sᴇᴀʀᴄʜ ᴀʙᴏᴜᴛ
ᴇxᴀᴍᴘʟᴇ: /anime Black Clover"""
        )
    ...
    if await (
        ...
        await client.send_photo(
            ...
            caption="Tʜɪs ᴀɴɪᴍᴇ ɪs ᴍᴀʀᴋᴇᴅ 18+ ᴀɴᴅ ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ"
        )
        return
    try:
        await client.send_photo(
...
async def manga_cmd(client: Client, message: Message, mdata: dict):
    ...
    if len(text) == 1:
        k = await message.reply_text(
"""Pʟᴇᴀsᴇ ɢɪᴠᴇ ᴀ ǫᴜᴇʀʏ ᴛᴏ sᴇᴀʀᴄʜ ᴀʙᴏᴜᴛ
ᴇxᴀᴍᴘʟᴇ: /manga The teasing master Takagi san"""
        )
        await asyncio.sleep(5)
        return await k.delete()
    query = text[1]
...
async def flex_cmd(client: Client, message: Message, mdata: dict):
    ...
    if "user" in query[0]:
        ...
        if not len(query) == 2:
            k = await message.reply_text(
"""Pʟᴇᴀsᴇ ɢɪᴠᴇ ᴀɴ ᴀɴɪʟɪsᴛ ᴜsᴇʀɴᴀᴍᴇ ᴛᴏ sᴇᴀʀᴄʜ ᴀʙᴏᴜᴛ
ᴇxᴀᴍᴘʟᴇ: /user DARKXSIDE78"""
)
            await asyncio.sleep(5)
            return await k.delete()
        else:
...
"""Pʟᴇᴀsᴇ ᴄᴏɴɴᴇᴄᴛ ʏᴏᴜʀ ᴀᴄᴄᴏᴜɴᴛ ғɪʀsᴛ ᴛᴏ ᴜsᴇ ᴛʜɪs ᴄᴍᴅ
Oʀ ᴄᴏɴɴᴇᴄᴛ ʏᴏᴜʀ ᴄʜᴀɴɴᴇʟ ᴡɪᴛʜ /connect ᴄᴍᴅ ɪғ ʏᴏᴜ ᴀʀᴇ ᴀɴᴏɴʏᴍᴏᴜs""",
...
async def top_tags_cmd(client: Client, message: Message, mdata: dict):
    ...
    if await (SFW_GRPS.find_one({"id": gid})) and str(result[0][1]) == "True":
        return await message.reply_text(
            'Nᴏ ɴsғᴡ sᴛᴜғғ ᴀʟʟᴏᴡᴇᴅ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ!!!'
        )
    msg, buttons = result
...
async def studio_cmd(client: Client, message: Message, mdata: dict):
    ...
    if find_gc is not None and 'studio' in find_gc['cmd_list'].split():
        return
    if len(text) == 1:
        x = await message.reply_text("Pʟᴇᴀsᴇ ɢɪᴠᴇ ᴀ ǫᴜᴇʀʏ ᴛᴏ sᴇᴀʀᴄʜ ᴀʙᴏᴜᴛ!!!\nᴇxᴀᴍᴘʟᴇ: /studio ufotable")
        await asyncio.sleep(5)
        await x.delete()
        return
    query = text[1]
    ...
    if len(result)==1:
        x = await message.reply_text("Nᴏ ʀᴇsᴜʟᴛs ғᴏᴜɴᴅ!!!")
        await asyncio.sleep(5)
...
async def airing_cmd(client: Client, message: Message, mdata: dict):
    ...
    if len(text) == 1:
        k = await message.reply_text(
"""Pʟᴇᴀsᴇ ɢɪᴠᴇ ᴀ ǫᴜᴇʀʏ ᴛᴏ sᴇᴀʀᴄʜ ᴀʙᴏᴜᴛ
ᴇxᴀᴍᴘʟᴇ: /airing Fumetsu no Anata e""")
        await asyncio.sleep(5)
        return await k.delete()
    query = text[1]
    qdb = rand_key()
...
async def auth_link_cmd(client, message: Message, mdata: dict):
    ...
    if mdata['chat']['id'] == user:
        text = "Cʟɪᴄᴋ ᴛʜᴇ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ ᴛᴏ ᴀᴜᴛʜᴏʀɪᴢᴇ ʏᴏᴜʀsᴇʟғ"
        if not os.environ.get('ANILIST_REDIRECT_URL'):
            text = """Fᴏʟʟᴏᴡ ᴛʜᴇ sᴛᴇᴘs ᴛᴏ ᴄᴏᴍᴘʟᴇᴛᴇ Aᴜᴛʜᴏʀɪᴢᴀᴛɪᴏɴ:
1. Cʟɪᴄᴋ ᴛʜᴇ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ
2. Aᴜᴛʜᴏʀɪᴢᴇ ᴛʜᴇ ᴀᴘᴘ ᴀɴᴅ ᴄᴏᴘʏ ᴛʜᴇ ᴀᴜᴛʜᴏʀɪᴢᴀᴛɪᴏɴ ᴄᴏᴅᴇ
3. Sᴇɴᴅ ᴛʜᴇ ᴄᴏᴘɪᴇᴅ ᴄᴏᴅᴇ ғᴏʟʟᴏᴡᴇᴅ ʙʏ ᴛʜᴇ ᴄᴏᴍᴍᴀɴᴅ /code
"""
        ...
        await message.reply_text(
            "Gᴏ ᴛᴏ ʙᴏᴛ ᴘᴍ ᴛᴏ ᴀᴜᴛʜᴏʀɪᴢᴇ ʏᴏᴜʀsᴇʟғ!!!",
...
setting_text = """
<b>Tʜɪs ᴀʟʟᴏᴡs ʏᴏᴜ ᴛᴏ ᴄʜᴀɴɢᴇ ɢʀᴏᴜᴘ sᴇᴛᴛɪɴɢs</b>
        
NSFW ᴛᴏɢɢʟᴇ sᴡɪᴛᴄʜᴇs ᴏɴ ғɪʟᴛᴇʀɪɴɢ ᴏғ 18+ ᴍᴀʀᴋᴇᴅ ᴄᴏɴᴛᴇɴᴛ.
Aɪʀɪɴɢ ɴᴏᴛɪғɪᴄᴀᴛɪᴏɴs ɴᴏᴛɪғɪᴇs ᴀʙᴏᴜᴛ ᴀɪʀɪɴɢ ᴏғ ᴀɴɪᴍᴇ ɪɴ ʀᴇᴄᴇɴᴛ.
Cʀᴜɴᴄʜʏʀᴏʟʟ ᴜᴘᴅᴀᴛᴇs ᴡɪʟʟ ᴛᴏɢɢʟᴇ ɴᴏᴛɪғɪᴄᴀᴛɪᴏɴs ᴀʙᴏᴜᴛ ʀᴇʟᴇᴀsᴇ ᴏғ ᴀɴɪᴍᴇs ᴏɴ ᴄʀᴜɴᴄʜʏʀᴏʟʟ sɪᴛᴇ.
Sᴜʙsᴘʟᴇᴀsᴇ ᴜᴘᴅᴀᴛᴇs ᴡɪʟʟ ᴛᴏɢɢʟᴇ ɴᴏᴛɪғɪᴄᴀᴛɪᴏɴs ᴀʙᴏᴜᴛ ʀᴇʟᴇᴀsᴇ ᴏғ ᴀɴɪᴍᴇs ᴏɴ sᴜʙsᴘʟᴇᴀsᴇ sɪᴛᴇ.
Cʟɪᴄᴋ Hᴇᴀᴅʟɪɴᴇs ʙᴜᴛᴛᴏɴ ᴛᴏ ᴇɴᴀʙʟᴇ ʜᴇᴀᴅʟɪɴᴇs. Yᴏᴜ ᴄᴀɴ ᴄʜᴏᴏsᴇ ғʀᴏᴍ ɢɪᴠᴇɴ sᴏᴜʀᴄᴇs."""
        ...
        await message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            text="Hᴇᴀᴅʟɪɴᴇs", callback_data=f"headlines_call_{cid}"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            text="Cʜᴀɴɢᴇ UI", callback_data=f"cui_call_{cid}"
                        )
                    ]
                ]
            )
...
async def man_code_cmd(client: Client, message: Message, _):
    ...
    if len(text) == 1:
        return await message.reply_text(
            'Sᴇɴᴅ ᴛʜᴇ ᴄᴏᴅᴇ ʏᴏᴜ ᴏʙᴛᴀɪɴᴇᴅ ғʀᴏᴍ ᴡᴇʙsɪᴛᴇ ᴀʟᴏɴɢ ᴡɪᴛʜ ᴛʜᴇ ᴄᴍᴅ'
        )
...
async def code_cmd(code: str, message: Message):
    ...
    if (await AUTH_USERS.find_one({"id": us_})):
        return await message.reply_text(
"""Yᴏᴜ ʜᴀᴠᴇ ᴀʟʀᴇᴀᴅʏ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ʏᴏᴜʀsᴇʟғ
Iғ ʏᴏᴜ ᴡɪsʜ ᴛᴏ ʟᴏɢᴏᴜᴛ sᴇɴᴅ /ʟᴏɢᴏᴜᴛ"""
        )
    ...
    if response.get("access_token"):
        ...
        await message.reply_text("Aᴜᴛʜᴏʀɪᴢᴀᴛɪᴏɴ Sᴜᴄᴄᴇssғᴜʟʟ!!!")
    else:
        await message.reply_text(
"""Pʟᴇᴀsᴇ ʀᴇᴛʀʏ ᴀᴜᴛʜᴏʀɪᴢᴀᴛɪᴏɴ ᴘʀᴏᴄᴇss!!!
Sᴏᴍᴇᴛʜɪɴɢ ᴜɴᴇxᴘᴇᴄᴛᴇᴅ ʜᴀᴘᴘᴇɴᴇᴅ"""
        )
...
Oʀ ᴄᴏɴɴᴇᴄᴛ ʏᴏᴜʀ ᴄʜᴀɴɴᴇʟ ᴡɪᴛʜ /connect ᴄᴍᴅ ɪғ ʏᴏᴜ ᴀʀᴇ ᴀɴᴏɴʏᴍᴏᴜs""",
    ...
    try:
        await client.send_photo(
            gid,
            result[0],
            caption="Cʜᴏᴏsᴇ ᴏɴᴇ ᴏғ ᴛʜᴇ ʙᴇʟᴏᴡ ᴏᴘᴛɪᴏɴs",
            reply_markup=btn
        )
    except (WebpageMediaEmpty, WebpageCurlFailed):
...
async def logout_cmd(client: Client, message: Message, mdata: dict):
    ...
    if gid == user:
        if (await AUTH_USERS.find_one({"id": user})):
            ...
            await message.reply_text("Lᴏɢɢᴇᴅ ᴏᴜᴛ!!!")
        else:
            await message.reply_text("Yᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴛᴏ ʙᴇɢɪɴ ᴡɪᴛʜ!!!")
    else:
        await message.reply_text(
            "Sᴇɴᴅ ᴛʜɪs ᴄᴍᴅ ɪɴ ᴘᴍ!!!",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(
...
async def list_tags_genres_cmd(client, message: Message, mdata: dict):
    ...
    if await (SFW_GRPS.find_one({"id": gid})) and 'nsfw' in text:
        return await message.reply_text('Nᴏ ɴsғᴡ ᴀʟʟᴏᴡᴇᴅ ʜᴇʀᴇ!!!')
    msg = (
...
async def page_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    if "No results Found" in result:
        await cq.answer("Nᴏ ᴍᴏʀᴇ ʀᴇsᴜʟᴛs ᴀᴠᴀɪʟᴀʙʟᴇ!!!", show_alert=True)
        return
    pic, msg = result[0], result[1][0]
    if media == "AIRING":
    ...
    if await (
        ...
        await cq.edit_message_media(
            InputMediaPhoto(
                no_pic[random.randint(0, 4)],
                caption="""
Tʜɪs ᴍᴀᴛᴇʀɪᴀʟ ɪs ᴍᴀʀᴋᴇᴅ 18+ ᴀɴᴅ ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ"""
            ),
                reply_markup=button
            )
        return
...
async def studio_pg_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    if len(result)==1:
        return await cq.answer("Nᴏ ᴍᴏʀᴇ ʀᴇsᴜʟᴛs ᴀᴠᴀɪʟᴀʙʟᴇ!!!", show_alert=True)
    msg, buttons = result[0], result[1]
...
async def studio_ani_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    if len(result)==1:
        return await cq.answer("Nᴏ ʀᴇsᴜʟᴛs ᴀᴠᴀɪʟᴀʙʟᴇ!!!", show_alert=True)
    msg, buttons = result[0], result[1]
    await cq.edit_message_text(msg, reply_markup=buttons)
...
async def nsfw_toggle_btn(client: Client, cq: CallbackQuery):
    ...
    if cus not in OWNER and k == MEMBER:
        await cq.answer(
            "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴇɴᴏᴜɢʜ ᴘᴇʀᴍɪssɪᴏɴs ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜɪs!!!",
            show_alert=True
        )
        return
    ...
    btns = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    text="Hᴇᴀᴅʟɪɴᴇs", callback_data=f"headlines_call_{query[2]}"
                )
            ],
            [
                InlineKeyboardButton(
                    text="Cʜᴀɴɢᴇ UI", callback_data=f"cui_call_{query[2]}"
                )
            ]
        ]
    )
...
async def list_favourites_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    if q[2] == "yes":
        btn.append(
            [InlineKeyboardButton("Bᴀᴄᴋ", callback_data=f"getusrbc_{q[3]}")]
        )
    else:
        btn.append(
            [
...
async def toggle_favourites_btn(
    ...
    if rslt == "ok":
        await cq.answer("Uᴘᴅᴀᴛᴇᴅ", show_alert=True)
    else:
        return await cq.answer("Fᴀɪʟᴇᴅ ᴛᴏ ᴜᴘᴅᴀᴛᴇ!!!", show_alert=True)
    result = (
        (
            await get_anime(
                {"id": idm},
...
async def update_anilist_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    if rslt == "ok":
        ...
        await cq.answer(
            "Sᴏᴍᴇᴛʜɪɴɢ ᴜɴᴇxᴘᴇᴄᴛᴇᴅ ʜᴀᴘᴘᴇɴᴇᴅ ᴀɴᴅ ᴏᴘᴇʀᴀᴛɪᴏɴ ғᴀɪʟᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ",
            show_alert=True
        )
        return
    result = (
...
async def additional_info_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    info = (
        "<b>Dᴇsᴄʀɪᴘᴛɪᴏɴ</b>"
        if kek == "desc"
        else "<b>Sᴇʀɪᴇs Lɪsᴛ</b>"
        if kek == "ls"
        else "<b>Cʜᴀʀᴀᴄᴛᴇʀs Lɪsᴛ</b>"
    )
    page = 0
    ...
    if result is None:
        await cq.answer('Nᴏ ᴅᴇsᴄʀɪᴘᴛɪᴏɴ ᴀᴠᴀɪʟᴀʙʟᴇ!!!', show_alert=True)
        return
    if "~!" in result and "!~" in result:
        result = re.sub(r"~!.*!~", "[Sᴘᴏɪʟᴇʀ]", result)
        spoiler = True
        button.append(
            [
                InlineKeyboardButton(
    ...
    if len(result) > 1000:
        result = result[:940] + "..."
        if spoiler is False:
            result += "\n\nFᴏʀ ᴍᴏʀᴇ ɪɴғᴏ ᴄʟɪᴄᴋ ʙᴇʟᴏᴡ ɢɪᴠᴇɴ ʙᴜᴛᴛᴏɴ"
            button.append([
                InlineKeyboardButton(
                    text="Mᴏʀᴇ Iɴғᴏ",
    ...
    if kek == 'char':
        ...
        add_ = f"\n\nTᴏᴛᴀʟ Cʜᴀʀᴀᴄᴛᴇʀs: {btndata['total']}"
    msg = f"{info}:\n\n{result+add_}"
    cbd = (
    ...
    button.append([InlineKeyboardButton(text="Bᴀᴄᴋ", callback_data=cbd)])
    try:
        await cq.edit_message_media(
            InputMediaPhoto(pic, caption=msg),
...
async def featured_in_btn(client: Client, cq: CallbackQuery, cdata: dict):
    ...
    if result[0] is False:
        ...
        req = None
        if result[0] is False:
            await cq.answer("Nᴏ Dᴀᴛᴀ Aᴠᴀɪʟᴀʙʟᴇ!!!", show_alert=True)
            return
    ...
    if total > 15:
        button.append([InlineKeyboardButton(
            text="Nᴇxᴛ", callback_data=f"lsca_{idm}_1_{qry}_{pg}_{auth}_{usr}"
        )])
    if req is not None:
        button.append([InlineKeyboardButton(
            text="Mᴀɴɢᴀ", callback_data=f"lscm_{idm}_0_{qry}_{pg}_{auth}_{usr}"
        )])
    button.append([InlineKeyboardButton(
        text="Bᴀᴄᴋ", callback_data=f"page_CHARACTER_{qry}_{pg}_{auth}_{usr}"
    )])
    try:
        await cq.edit_message_media(
            InputMediaPhoto(pic, caption=msg),
...
headlines_text = '''
Tᴜʀɴ LɪᴠᴇCʜᴀʀᴛ ᴏᴘᴛɪᴏɴ ᴏɴ ᴛᴏ ɢᴇᴛ ɴᴇᴡs ғᴇᴇᴅs ғʀᴏᴍ ʟɪᴠᴇᴄʜᴀʀᴛ.ᴍᴇ.
Tᴜʀɴ MʏAɴɪᴍᴇLɪsᴛ ᴏᴘᴛɪᴏɴ ᴏɴ ᴛᴏ ɢᴇᴛ ɴᴇᴡs ғᴇᴇᴅs ғʀᴏᴍ ᴍʏᴀɴɪᴍᴇʟɪsᴛ.ɴᴇᴛ.

Fᴏʀ Aᴜᴛᴏ Pɪɴ ᴀɴᴅ Aᴜᴛᴏ Uɴᴘɪɴ ғᴇᴀᴛᴜʀᴇs, ɢɪᴠᴇ ᴛʜᴇ ʙᴏᴛ "Pɪɴ Mᴇssᴀɢᴇ" ᴀɴᴅ "Dᴇʟᴇᴛᴇ Mᴇssᴀɢᴇ" ᴘᴇʀᴍs.
Aᴜᴛᴏ Uɴᴘɪɴ ᴄᴀɴ ʙᴇ ᴄᴜsᴛᴏᴍɪᴢᴇᴅ, ᴄʟɪᴄᴋ ᴏɴ ᴛʜᴇ ʙᴜᴛᴛᴏɴ ᴛᴏ sᴇᴇ ᴀᴠᴀɪʟᴀʙʟᴇ ᴏᴘᴛɪᴏɴs.
'''
...
async def headlines_btn(client: Client, cq: CallbackQuery):
    ...
    btn = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(text="Bᴀᴄᴋ", callback_data=f"settogl_call_{gid}")
            ]
        ]
    )
...
async def auto_unpin(client: Client, cq: CallbackQuery):
    ...
    if data:
        ...
        cancel = True
    if cancel:
        return await cq.answer(f"Pʟᴇᴀsᴇ ᴇɴᴀʙʟᴇ {srcname} ᴀɴᴅ Aᴜᴛᴏ Pɪɴ ᴏᴘᴛɪᴏɴ ғᴏʀ ᴛʜᴇᴍ!!!", show_alert=True)
    setting = None
    if qry=="call":
        pass
    elif qry == "None":
    ...
    if len(row)!=0:
        btn.append(row)
    btn.append([InlineKeyboardButton("Bᴀᴄᴋ", callback_data=f"headlines_call_{gid}")])
    ...
    await cq.edit_message_text(
        f"Aᴜᴛᴏ Uɴᴘɪɴ ᴏᴘᴛɪᴏɴs ғᴏʀ {srcname}\nCᴜʀʀᴇɴᴛʟʏ sᴇᴛ ᴛᴏ: {unpindata}",
        reply_markup=InlineKeyboardMarkup(btn)
    )
    await cq.answer()
...
async def change_ui_btn(client: Client, cq: CallbackQuery):
    ...
    btn.append([InlineKeyboardButton(
        text="Bᴀᴄᴋ", callback_data=f"settogl_call_{gid}")])
    ...
    if await GUI.find_one({"_id": gid}):
        ...
        bl = data["bl"]
        cs = data["cs"]
    text = f"""Sᴇʟᴇᴄᴛᴇᴅ ʙᴜʟʟᴇᴛ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ: {bl}
Sᴇʟᴇᴄᴛᴇᴅ ᴛᴇxᴛ ᴄᴀsᴇ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ: {cs}"""
