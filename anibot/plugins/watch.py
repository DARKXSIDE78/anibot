...
async def get_watch_order(client: Client, message: Message, mdata: dict):
    ...
    if len(x)==1:
        await message.reply_text("Nᴏᴛʜɪɴɢ ɢɪᴠᴇɴ ᴛᴏ sᴇᴀʀᴄʜ ғᴏʀ!!!")
        return
    try:
    ...
    data = get_wols(x[1])
    msg = f"Fᴏᴜɴᴅ ʀᴇʟᴀᴛᴇᴅ ᴀɴɪᴍᴇs ғᴏʀ ᴛʜᴇ ǫᴜᴇʀʏ {x[1]}"
    buttons = []
    if data == []:
        await client.send_message(gid, 'Nᴏ ʀᴇsᴜʟᴛs ғᴏᴜɴᴅ!!!')
        return
    for i in data:
        buttons.append(
            [
...
async def watch_(client: anibot, cq: CallbackQuery, cdata: dict):
    ...
    button.append([
        InlineKeyboardButton("Bᴀᴄᴋ", callback_data=f"wol_{qry}_{user}")
    ])
...
async def wls(client: anibot, cq: CallbackQuery, cdata: dict):
    ...
    data = get_wols(qry)
    msg = f"Fᴏᴜɴᴅ ʀᴇʟᴀᴛᴇᴅ ᴀɴɪᴍᴇs ғᴏʀ ᴛʜᴇ ǫᴜᴇʀʏ {qry}"
    buttons = []
    for i in data:
        buttons.append(
            [
