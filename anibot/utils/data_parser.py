...
🎬 {trailer_link}
📖 <a href="{surl}">Sʏɴᴏᴘsɪs</a>
📖 <a href="{url}">Oғғɪᴄɪᴀʟ Sɪᴛᴇ</a>
<a href="https://t.me/{bot}?start=anirec_{idm}">Rᴇᴄᴏᴍᴍᴇɴᴅᴀᴛɪᴏɴs</a>
...
async def get_studios(qry, page, user, duser = None, auth: bool = False):
    ...
    if auth:
        btns.append([
            InlineKeyboardButton(
                "Rᴇᴍᴏᴠᴇ ғʀᴏᴍ Fᴀᴠs" if isFav else "Add To Favs",
    ...
    if pi is False:
        ...
        if int(page)==1:
            btns.append([
                InlineKeyboardButton(
                    "Nᴇxᴛ", callback_data=f"pgstudio_2_{qry}_{auth}_{duser}"
                )
            ])
        else:
            btns.append(
                [
                    InlineKeyboardButton(
                        "Pʀᴇᴠ", callback_data=f"pgstudio_{page-1}_{qry}_{auth}_{duser}"
                    ),
                    InlineKeyboardButton(
                        "Nᴇxᴛ", callback_data=f"pgstudio_{page+1}_{qry}_{auth}_{duser}"
                    )
                ]
            )
...
async def get_studio_animes(id_, page, qry, rp, user, duser = None, auth: bool = False):
    ...
    msg = f"Lɪsᴛ ᴏғ ᴀɴɪᴍᴇs ʙʏ {result['data']['Studio']['name']} studio\n"
    for i in data:
        msg += (
            f"\n⚬ `{i['node']['title']['romaji']}`"
    ...
    if pi['hasNextPage'] is False:
        if int(page)==1:
            btns.append([
                InlineKeyboardButton(
                    "Bᴀᴄᴋ", callback_data=f"pgstudio_{rp}_{qry}_{auth}_{duser}"
                )
            ])
            return msg, btns
        else:
        ...
        if int(page)==1:
            btns.append([
                InlineKeyboardButton(
                    "Nᴇxᴛ", callback_data=f"stuani_2_{id_}_{rp}_{qry}_{auth}_{duser}"
                )
            ])
        else:
            btns.append([
...
async def get_user_activity(id_, user, duser = None):
    ...
    if duser is None:
        duser = user
    btn = [[InlineKeyboardButton("Bᴀᴄᴋ", callback_data=f"getusrbc_{duser}")]]
    return [
...
async def get_recommendations(id_):
    ...
    if rc_ls == []:
        return "Nᴏ Rᴇᴄᴏᴍᴍᴇɴᴅᴀᴛɪᴏɴs ᴀᴠᴀɪʟᴀʙʟᴇ ʀᴇʟᴀᴛᴇᴅ ᴛᴏ ɢɪᴠᴇɴ ᴀɴɪᴍᴇ!!!"
    outstr = "Rᴇᴄᴏᴍᴍᴇɴᴅᴇᴅ ᴀɴɪᴍᴇs:\n\n"
    for i in rc_ls:
        outstr += (
            f"**{i[0]}**\n ➥[Sʏɴᴏᴘsɪs]"
...
async def get_top_animes(gnr: str, page, user):
    ...
    query = TOP_QUERY
    msg = f"Tᴏᴘ ᴀɴɪᴍᴇs ғᴏʀ ɢᴇɴʀᴇ `{gnr.capitalize()}`:\n\n"
    if gnr=="None":
        query = ALLTOP_QUERY
        vars_ = {"page": int(page)}
        msg = f"Tᴏᴘ ᴀɴɪᴍᴇs:\n\n"
    ...
    if len(result['data']['Page']['media'])==0:
        query = TOPT_QUERY
        msg = f"Tᴏᴘ ᴀɴɪᴍᴇs ғᴏʀ ᴛᴀɢ `{gnr.capitalize()}`:\n\n"
        result = await return_json_senpai(query, vars_, auth=False, user=user)
        if len(result['data']['Page']['media'])==0:
    ...
    msg += f"\nTᴏᴛᴀʟ ᴀᴠᴀɪʟᴀʙʟᴇ ᴀɴɪᴍᴇs: `{data['pageInfo']['total']}`"
    btn = []
    if int(page)==1:
...
async def get_user_favourites(id_, user, req, page, sighs, duser = None):
    ...
    msg = (
        "Fᴀᴠᴏᴜʀɪᴛᴇ Aɴɪᴍᴇs:\n\n" if req=="ANIME" 
        else "Fᴀᴠᴏᴜʀɪᴛᴇ Cʜᴀʀᴀᴄᴛᴇʀs:\n\n" if req=="CHAR" 
        else "Fᴀᴠᴏᴜʀɪᴛᴇ Mᴀɴɢᴀ:\n\n"
    )
    for i in data["edges"]:
        node_name = (
    ...
    btn.append([
        InlineKeyboardButton(
            "Bᴀᴄᴋ", callback_data=f"myfavs_{id_}_{sighs}_{user}"
        )
    ])
    return [
        f"https://img.anili.st/user/{id_}?a=({time.time()})",
...
async def get_anime(
    ...
    if data["nextAiringEpisode"]:
        ...
        air_on += f" | {eps}{th} ᴇᴘs"
    if air_on is None:
        eps_ = f"` | `{episodes} ᴇᴘs" if episodes is not None else ""
    ...
    if data["trailer"] and data["trailer"]["site"] == "youtube":
        trailer_link = (
            f"<a href='https://youtu.be/{data['trailer']['id']}'>Tʀᴀɪʟᴇʀ</a>"
        )
...
async def get_character(query, page, auth: bool = False, user: int = None):
    ...
    if len(va)>1:
        lva = va.pop()
    sva = (
        f"\n**Vᴏɪᴄᴇ Aᴄᴛᴏʀs:** {', '.join(va)}"
        +f"{' ᴀɴᴅ '+lva if lva is not None else ''}\n" if va!= []
        else ""
    )
    cap_text = f"""
__{native}__
...
**ID:** {id_}
{sva}
<a href='{site_url}'>Vɪsɪᴛ Wᴇʙsɪᴛᴇ</a>"""
    hasNextPage = result["data"]["Page"]["pageInfo"]["hasNextPage"]
...
async def browse_(qry: str):
    ...
    out = f'{qry.capitalize()} ᴀɴɪᴍᴇs ɪɴ {s} {y}:\n\n'
    return out + "\n".join(ls[:20])
...
async def get_manga(
    ...
    if len(synopsis) > 500:
        description += f"..."
        description_s = (
            f"[Cʟɪᴄᴋ ғᴏʀ ᴍᴏʀᴇ ɪɴғᴏ](https://t.me/{BOT_NAME.replace('@', '')}"
    ...
    if auth is True:
        ...
        if in_list is not None:
            ...
            in_ls_stts = in_list['status']
            in_ls_score = (
                f" ᴀɴᴅ sᴄᴏʀᴇᴅ {in_list['score']}" if in_list['score']!=0
                else ""
            )
            user_data = (
...
async def get_airing(qry, ind: int, auth: bool = False, user: int = None):
    ...
    out += user_data
    if air_on:
        out += f"Aɪʀɪɴɢ Eᴘɪsᴏᴅᴇ `{episode}{th}` in `{air_on}`"
    site = data["siteUrl"]
    return [
        coverImg, out
    ], [
...
Tᴏᴛᴀʟ Aɴɪᴍᴇ Wᴀᴛᴄʜᴇᴅ: `{anime['count']}`
Tᴏᴛᴀʟ Eᴘɪsᴏᴅᴇ Wᴀᴛᴄʜᴇᴅ: `{anime['episodesWatched']}`
Tᴏᴛᴀʟ Tɪᴍᴇ Sᴘᴇɴᴛ: `{anime['minutesWatched']}`
Aᴠᴇʀᴀɢᴇ Sᴄᴏʀᴇ: `{anime['meanScore']}`

**Mᴀɴɢᴀ Sᴛᴀᴛs**:

Tᴏᴛᴀʟ Mᴀɴɢᴀ Rᴇᴀᴅ: `{manga['count']}`
Tᴏᴛᴀʟ Cʜᴀᴘᴛᴇʀs Rᴇᴀᴅ: `{manga['chaptersRead']}`
Tᴏᴛᴀʟ Vᴏʟᴜᴍᴇs Rᴇᴀᴅ: `{manga['volumesRead']}`
Aᴠᴇʀᴀɢᴇ Sᴄᴏʀᴇ: `{manga['meanScore']}`
""" 
    btn = []
    if not "user" in req:
        btn.append([
...
async def get_scheduled(x: int = 9):
    ...
    out = f"Sᴄʜᴇᴅᴜʟᴇᴅ ᴀɴɪᴍᴇs ғᴏʀ {day.capitalize()}\n\n"
...
def get_wo(x: int, page: int):
    ...
    msg = "Wᴀᴛᴄʜ ᴏʀᴅᴇʀ ғᴏʀ ᴛʜᴇ ɢɪᴠᴇɴ ǫᴜᴇʀʏ ɪs:\n\n"
    out = []
    for i in data:
